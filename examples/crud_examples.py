...
class CRUDExamples:
    ...
    async def example_filtering_and_search(self, db):
        ...
        active_customers = await self.user_crud.filter(
            db,
            filters={"role": UserRole.CUSTOMER, "is_active": True},
            limit=10
        )
    ...
    async def example_relationships(self, db):
        ...
        account_with_transactions = await self.account_crud.get(
            db,
            accounts[0].id,
            joins=[Account.to_transactions]
        )
        print(f"✅ Account with {len(account_with_transactions.to_transactions)} transactions")
